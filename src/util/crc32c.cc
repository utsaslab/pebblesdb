// Copyright (c) 2014 The LevelDB and HyperLevelDB Authors. All rights reserved.
...
static uint32_t crc32_software(uint32_t init_crc, const char* _data, size_t n) {
...
static uint32_t crc32_sse42_simple(uint32_t init_crc, const char* data, size_t n) {
  uint64_t crc = init_crc ^ CRC_FFs;
  for (size_t i = 0; i < n; ++i) {
    __asm__ __volatile__("crc32b %2, %0\n\t" : "=r"(crc) : "r"(crc), "r"(data[i]) : );
  }
  crc ^= CRC_FFs;
  return crc;
}

static uint32_t crc32_sse42_quads(uint32_t init_crc, const char* data, size_t n) {
  uint64_t crc = init_crc ^ CRC_FFs;
  ...
  for (size_t i = 0; i < body; ++i) {
    __asm__ __volatile__("crc32q %2, %0\n\t" : "=r"(crc) : "r"(crc), "r"(body_ptr[i]) : );
  }
  const size_t offset = n - tail;
  for (size_t i = 0; i < tail; ++i) {
    __asm__ __volatile__("crc32b %2, %0\n\t" : "=r"(crc) : "r"(crc), "r"(data[offset + i]) : );
  }
  crc ^= CRC_FFs;
  return crc;
}

typedef uint32_t (*crc32c_func_t)(uint32_t init_crc, const char* data, size_t n);
...
extern uint32_t Extend(uint32_t init_crc, const char* data, size_t n) {
  return crc32c_func(init_crc, data, n);
}
